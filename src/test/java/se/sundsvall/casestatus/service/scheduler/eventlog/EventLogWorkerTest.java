...
class EventLogWorkerTest {
	...
	void updateSupportManagementStatuses() {
		...
		when(eventPageMock.getContent()).thenReturn(List.of(
			new Event().logKey("1")
				.metadata(List.of(new Metadata().key("Status").value(internalStatus),
					new Metadata().key("Channel").value(EXTERNAL_CHANNEL_E_SERVICE),
					new Metadata().key("CaseId").value(caseId1))),
			new Event().logKey("2")
				.metadata(List.of(new Metadata().key("Status").value(internalStatus),
					new Metadata().key("Channel").value(EXTERNAL_CHANNEL_E_SERVICE),
					new Metadata().key("CaseId").value(caseId2)))));
		...
		when(eventlogClientMock.getEvents(eq(municipalityId), any(PageRequest.class), filterArgumentCaptor.capture()))
		...
		verify(oepIntegratorClientMock).setStatus(eq(municipalityId), eq(InstanceType.EXTERNAL), eq(caseId1),
			any(CaseStatusChangeRequest.class));
		verify(oepIntegratorClientMock).setStatus(eq(municipalityId), eq(InstanceType.EXTERNAL), eq(caseId2),
			any(CaseStatusChangeRequest.class));
		verify(statusesRepositoryMock, times(2)).findBySupportManagementStatus(internalStatus);
		assertThat(filterArgumentCaptor.getValue()).isEqualTo("message:'Ärendet har uppdaterats.' and created > '"
			+ executionInformationEntity.getLastSuccessfulExecution().minus(Duration.parse("PT5S"))
	...
	void testUpdateSupportManagementStatusesWithEmptyLogKeys() {
		...
		assertThat(result).isTrue();
		verify(eventlogClientMock).getEvents(eq(municipalityId), any(PageRequest.class),
	...
	void updateSupportManagementStatusesWithoutCaseId() {
		...
		when(eventlogClientMock.getEvents(eq(municipalityId), any(PageRequest.class), anyString()))
			.thenReturn(eventPageMock);
		when(statusesRepositoryMock.findBySupportManagementStatus("SomeStatus"))
			.thenReturn(Optional.of(StatusesEntity.builder().withOepStatus("OepStatus").build()));
	...
	void updateStatusWithMismatchingSupportManagementStatuses() {
		...
		when(eventPageMock.getContent()).thenReturn(List.of(new Event().logKey("1")
			.metadata(List.of(new Metadata().key("Status").value(internalStatus),
				new Metadata().key("Channel").value(EXTERNAL_CHANNEL_E_SERVICE),
				new Metadata().key("CaseId").value("case-1")))));
		when(eventPageMock.hasNext()).thenReturn(false);
		when(eventlogClientMock.getEvents(eq(municipalityId), any(PageRequest.class), anyString()))
			.thenReturn(eventPageMock);
		...
		assertThat(result).isTrue();
		verify(statusesRepositoryMock).findBySupportManagementStatus(internalStatus);
		verifyNoInteractions(oepIntegratorClientMock);
	}
	...
	void updateSupportManagementStatusesWithOepIntegratorFailure() {
		...
		when(eventPageMock.getContent()).thenReturn(List.of(new Event().logKey("1")
			.metadata(List.of(new Metadata().key("Status").value(internalStatus),
				new Metadata().key("Channel").value(EXTERNAL_CHANNEL_E_SERVICE),
				new Metadata().key("CaseId").value(caseId)))));
		when(eventPageMock.hasNext()).thenReturn(false);
		when(statusesRepositoryMock.findBySupportManagementStatus(internalStatus)).thenReturn(Optional.of(statuses));
		when(eventlogClientMock.getEvents(eq(municipalityId), any(PageRequest.class), anyString()))
			.thenReturn(eventPageMock);
		when(oepIntegratorClientMock.setStatus(anyString(), any(), anyString(), any()))
		...
		verify(oepIntegratorClientMock).setStatus(eq(municipalityId), eq(InstanceType.EXTERNAL), eq(caseId),
			any(CaseStatusChangeRequest.class));
		verify(consumerMock).accept("Failed to update openE status for case with external ID " + caseId);
		verify(messagingIntegrationMock).sendSlackMessage(eq(municipalityId), contains(caseId));
	}
	...
	void updateSupportManagementStatusesWithInternalChannel() {
		...
		when(eventPageMock.getContent()).thenReturn(List.of(new Event().logKey("1")
			.metadata(List.of(new Metadata().key("Status").value(internalStatus),
				new Metadata().key("Channel").value(INTERNAL_CHANNEL_E_SERVICE),
				new Metadata().key("CaseId").value(caseId)))));
		when(eventPageMock.hasNext()).thenReturn(false);
		when(statusesRepositoryMock.findBySupportManagementStatus(internalStatus)).thenReturn(Optional.of(statuses));
		when(eventlogClientMock.getEvents(eq(municipalityId), any(PageRequest.class), anyString()))
		...
		assertThat(result).isTrue();
		verify(oepIntegratorClientMock).setStatus(eq(municipalityId), eq(InstanceType.INTERNAL), eq(caseId),
			any(CaseStatusChangeRequest.class));
	}
	...
	void updateSupportManagementStatusesWithInvalidChannel() {
		...
		when(eventPageMock.getContent()).thenReturn(
			List.of(new Event().logKey("1").metadata(List.of(new Metadata().key("Status").value("SomeStatus"),
				new Metadata().key("Channel").value("PHONE"), new Metadata().key("CaseId").value("case-1")))));
		when(eventPageMock.hasNext()).thenReturn(false);
		when(eventlogClientMock.getEvents(eq(municipalityId), any(PageRequest.class), anyString()))
			.thenReturn(eventPageMock);
	...
	void updateOepCase_success() {
		...
		assertThat(filterArgumentCaptor.getValue()).contains("message ~ 'Status updated to'")
			.contains("owner: 'CaseManagement'").contains("sourceType: 'Errand'").contains("type: 'UPDATE'")
			.contains("created > '"
				+ executionInformationEntity.getLastSuccessfulExecution().minus(Duration.parse("PT5S")));
	}
	...
	void updateOepCase_nullExternalCaseIdSkipsEvent() {
		...
		when(eventPageMock.getContent()).thenReturn(List
			.of(new Event().logKey("12345").metadata(List.of(new Metadata().key("Status").value(statusValue)))));
		when(eventPageMock.hasNext()).thenReturn(false);
		when(eventlogClientMock.getEvents(eq(municipalityId), any(PageRequest.class), anyString()))
			.thenReturn(eventPageMock);
		when(statusesRepositoryMock.findByCaseManagementStatus(statusValue)).thenReturn(Optional.of(statuses));
	...
	void updateOepCase_multipleEventsAllProcessed() {
		...
		when(eventPageMock.getContent()).thenReturn(List.of(
			new Event().logKey("1")
				.metadata(List.of(new Metadata().key("Status").value(statusValue1),
					new Metadata().key("ExternalCaseId").value(externalCaseId1))),
			new Event().logKey("2").metadata(List.of(new Metadata().key("Status").value(statusValue2),
		...
		assertThat(result).isTrue();
		verify(oepIntegratorClientMock).setStatus(eq(municipalityId), eq(InstanceType.EXTERNAL), eq(externalCaseId1),
			any(CaseStatusChangeRequest.class));
		verify(oepIntegratorClientMock).setStatus(eq(municipalityId), eq(InstanceType.EXTERNAL), eq(externalCaseId2),
			any(CaseStatusChangeRequest.class));
	...
	void updateOepCase_failureContinuesProcessing() {
		...
		assertThat(slackMessageCaptor.getValue()).startsWith("CaseStatus [junit] - RequestID: ")
			.contains("Failed to set status 'OepStatus1' for case with external ID 'ext-1'. Error: 'OEP error'");
		verify(consumerMock, times(1))
			.accept("Failed to update openE status for case with external ID " + externalCaseId1);
	}
}
