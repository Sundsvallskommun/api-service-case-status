...
class PartyConfigurationTest {
	...
	void testFeignBuilderCustomizer() {
		...
		try (final MockedStatic<FeignMultiCustomizer> feignMultiCustomizerMock = Mockito.mockStatic(FeignMultiCustomizer.class)) {
			...
			verify(clientRegistrationRepositoryMock).findByRegistrationId(PartyConfiguration.CLIENT_ID);
			...
			AssertionsForClassTypes.assertThat(errorDecoderCaptor.getValue()).hasFieldOrPropertyWithValue("integrationName", PartyConfiguration.CLIENT_ID);
