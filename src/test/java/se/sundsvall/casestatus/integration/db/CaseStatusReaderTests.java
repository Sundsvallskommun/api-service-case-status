...
class CaseStatusReaderTests {
    ...
    void getCaseManagementOpenEStatus() {
        ...
        var result = caseStatusReader.getCaseManagementOpenEStatus("someId");

        assertThat(result).isNotNull();

        verify(mockJdbcTemplate, times(1)).query(any(String.class), anyMap(), ArgumentMatchers.<ResultSetExtractor<String>>any());
    ...
    void getIncidentOpenEStatus() {
        ...
        var result = caseStatusReader.getIncidentOpenEStatus(456);
    ...
    void getMapCaseTypeEnumText() {
        ...
        var result = caseStatusReader.getMapCaseTypeEnumText("someEnumValue");
    ...
    void mapOpenEId() throws SQLException {
        ...
        var result = caseStatusReader.mapOpenEId(mockResultSet);

        assertThat(result).isNotNull();
        assertThat(result).isEqualTo("someId");
    }
    ...
    void mapCaseTypeEnumText() throws SQLException {
        ...
        var result = caseStatusReader.mapCaseTypeEnumText(mockResultSet);
    ...
    void getExternalCaseIdStatus() {
        ...
        var result = caseStatusReader.getExternalCaseIdStatus("someExternalCaseId");

        assertThat(result).isNotNull();

        verify(mockJdbcTemplate, times(1)).query(any(String.class), anyMap(), ArgumentMatchers.<ResultSetExtractor<CachedCaseStatus>>any());
        verifyNoMoreInteractions(mockJdbcTemplate);
    }
    ...
    void getOrganizationStatuses() {
        ...
        var result = caseStatusReader.getOrganizationStatuses("someExternalCaseId");

        assertThat(result).isNotNull().hasSize(2);

        verify(mockJdbcTemplate, times(1)).query(any(String.class), anyMap(), ArgumentMatchers.<RowMapper<CachedCaseStatus>>any());
        verifyNoMoreInteractions(mockJdbcTemplate);
    }

    @Test
    void mapRow() throws SQLException {
        var result = caseStatusReader.mapRow(mockResultSet(), 0);
    ...
    void mapToCachedCaseStatus() throws SQLException {
        var result = caseStatusReader.mapToCachedCaseStatus(mockResultSet());
    ...
    private ResultSet mockResultSet() throws SQLException {
        var mockResultSet = mock(ResultSet.class);
        when(mockResultSet.next()).thenReturn(true);
        when(mockResultSet.getString(eq("FlowInstanceID"))).thenReturn("someFlowInstanceId");
        when(mockResultSet.getString(eq("ErrandType"))).thenReturn("someErrandType");
        when(mockResultSet.getString(eq("Status"))).thenReturn("someStatus");
        when(mockResultSet.getString(eq("FirstSubmitted"))).thenReturn("someFirstSubmittedValue");
        when(mockResultSet.getString(eq("LastStatusChange"))).thenReturn("someLastStatusChangeValue");
